...
export default class SlotTest extends NavigationMixin(LightningElement) {
    ...
    recordToDelete = null;
    recordNameToDelete = 'this record';
    ...
    async loadRelatedActivity() {
        ...
        try {
            ...
            modifiedResponse.forEach(rec => {
                rec.url = this.buildRecordUrl(rec.Id);
    ...
    async loadDataWithARL(appendRecords = false) {
        ...
        if (response?.fields) {
            // Store the related object type for permission checks
            this.relatedObjectType = response.relatedObjectType;
    ...
    async checkDeletePermission() {
        try {
            if (!this.enableRecordDeletion) {
                this.canDeleteRecords = false;
                return;
            }

            // Check permission on the RELATED object type (child), not the parent
            const objectTypeToCheck = this.relatedObjectType || this.detectedObjectType;

            if (!objectTypeToCheck) {
                this.canDeleteRecords = false;
                return;
            }

            this.canDeleteRecords = await canDeleteObject({
                objectApiName: objectTypeToCheck
            });

            this.debugLog('Delete permission check:', {
                parentObjectType: this.detectedObjectType,
                relatedObjectType: this.relatedObjectType,
                checkingObjectType: objectTypeToCheck,
    ...
    handleDeleteRecord(recordId) {
        ...
        // Find the record to get its name for the modal
        const record = this.allRecords.find(r => r.Id === recordId);
        this.recordNameToDelete = this.getRecordDisplayName(record);
        this.recordToDelete = recordId;
        this.showDeleteModal = true;

        this.debugLog('Showing delete confirmation for record:', recordId);
    }
    ...
    handleCancelDelete() {
        ...
        this.recordNameToDelete = 'this record';
    }
    ...
    async handleConfirmDelete() {
        ...
        this.debugLog('Confirming deletion for record:', this.recordToDelete);

        try {
            await deleteRecord({ recordId: this.recordToDelete });

            // Remove from allRecords array
            this.allRecords = this.allRecords.filter(record => record.Id !== this.recordToDelete);
            ...
            this.debugLog('Record deleted successfully');
